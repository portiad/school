...
isbn, and price of the book as a floating-point number, price, 
as attributes. The constructor should accept the ISBN number (a string) 
as the first argument and price as second argument, 
and should raise ArgumentError (one of Ruby's built-in exception types) 
if the ISBN number is the empty string or if the price is less than or 
equal to zero.
...
Please put the class in a single file and upload it using the 
upload form below:
=end
...
class BookInStock
	...
	def initialize (isbn, price)
    	if isbn.length == 0 || price <= 0
      		raise ArgumentError.new('Please enter in ISBN and a price greater than $0.')  
		end 
		@isbn = isbn
		@price = price
	end
